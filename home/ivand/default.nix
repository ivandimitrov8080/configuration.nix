  ...
  systemd.user = {
    ...
    services = {
      wpd = {
        Service = {
          Environment = [
            "PATH=${pkgs.nushell}/bin:${pkgs.xdg-user-dirs}/bin:${pkgs.swaybg}/bin"
          ];
          ExecStart = [ "nu -c 'swaybg -i (ls ((xdg-user-dir PICTURES) | path join 'bg') | get name | last)'" ];
        };
      };
      bingwp = {
        Service = {
      ...
      rbingwp = {
        Service = {
          Type = "oneshot";
          ExecStart = [ "${pkgs.systemd}/bin/systemctl --user restart bingwp.service && ${pkgs.systemd}/bin/systemctl --user restart wpd.service" ];
        };
      };
    };
  };
